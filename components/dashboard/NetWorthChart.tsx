...
export function NetWorthChart({ history, netWorth }: { history: NetWorthPoint[]; netWorth: number }) {
  ...
  return (
    <Card className="h-full">
      <CardHeader className="flex-row items-baseline justify-between">
        ...
        <span className="text-xl font-semibold tabular-nums">{formatCurrency(netWorth)}</span>
      </CardHeader>
      <CardContent className="h-48">
        {chartData.length < 2 ? (
          <div className="flex h-full items-center justify-center text-sm text-muted-foreground">
            Check back tomorrow to see your net worth trend build up.
          </div>
        ) : (
